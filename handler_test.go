...
func TestGoZeroMysqlHandlerError(t *testing.T) {
	...
	if _, err := ctl(nil).Filter(Cond{}).Where("").FindOne(); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != fmt.Sprintf(queryset.FilterOrWhereError, "Filter") {
		t.Errorf("got %q, want %q", err.Error(), fmt.Sprintf(queryset.FilterOrWhereError, "Filter"))
	}

	ctx := context.Background()

	if _, err := ctl(ctx).Exclude(Cond{}).Where("").FindOne(); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != fmt.Sprintf(queryset.FilterOrWhereError, "Exclude") {
		t.Errorf("got %q, want %q", err.Error(), fmt.Sprintf(queryset.FilterOrWhereError, "Exclude"))
	}

	// FindOne unsupported operations
	if _, err := ctl(ctx).Select("").FindOne(); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "[Select] not supported for FindOne" {
		t.Errorf("got %q, want %q", err.Error(), "[Select] not supported for FindOne")
	}
	if _, err := ctl(ctx).Having("").FindOne(); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "[Having] not supported for FindOne" {
		t.Errorf("got %q, want %q", err.Error(), "[Having] not supported for FindOne")
	}

	// FindAll unsupported operations
	if _, err := ctl(ctx).GroupBy("").Select("").FindAll(); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "[Select] not supported for FindAll" {
		t.Errorf("got %q, want %q", err.Error(), "[Select] not supported for FindAll")
	}

	if _, err := ctl(ctx).Limit(0, 1).FindAll(); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != fmt.Errorf(MustBeCalledError, "Limit", "OrderBy").Error() {
		t.Errorf("got %q, want %q", err.Error(), fmt.Errorf(MustBeCalledError, "Limit", "OrderBy").Error())
	}

	// Create unsupported operations
	if id, err := ctl(ctx).Filter(Cond{}).Create(map[string]any{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if id != 0 {
		t.Errorf("got id %d, want 0", id)
	} else if err.Error() != "[Filter] not supported for Create" {
		t.Errorf("got %q, want %q", err.Error(), "[Filter] not supported for Create")
	}

	if id, err := ctl(ctx).Filter(Cond{}).Where("").Create(map[string]any{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if id != 0 {
		t.Errorf("got id %d, want 0", id)
	} else if err.Error() != "[Filter, Where] not supported for Create" {
		t.Errorf("got %q, want %q", err.Error(), "[Filter, Where] not supported for Create")
	}

	if id, err := ctl(ctx).Filter(Cond{}).Where("").OrderBy("").Create(map[string]any{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if id != 0 {
		t.Errorf("got id %d, want 0", id)
	} else if err.Error() != "[Filter, Where, OrderBy] not supported for Create" {
		t.Errorf("got %q, want %q", err.Error(), "[Filter, Where, OrderBy] not supported for Create")
	}

	if id, err := ctl(ctx).Filter(Cond{}).Where("").OrderBy("").GroupBy("").Create(map[string]any{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if id != 0 {
		t.Errorf("got id %d, want 0", id)
	} else if err.Error() != "[Filter, Where, OrderBy, GroupBy] not supported for Create" {
		t.Errorf("got %q, want %q", err.Error(), "[Filter, Where, OrderBy, GroupBy] not supported for Create")
	}

	if id, err := ctl(ctx).Filter(Cond{}).Where("").OrderBy("").GroupBy("").Select("").Create(map[string]any{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if id != 0 {
		t.Errorf("got id %d, want 0", id)
	} else if err.Error() != "[Filter, Where, Select, OrderBy, GroupBy] not supported for Create" {
		t.Errorf("got %q, want %q", err.Error(), "[Filter, Where, Select, OrderBy, GroupBy] not supported for Create")
	}

	if id, err := ctl(ctx).Filter(Cond{}).Create(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if id != 0 {
		t.Errorf("got id %d, want 0", id)
	} else if err.Error() != "[Filter] not supported for Create" {
	...
	if id, err := ctl(ctx).Filter(Cond{}).Where("").Create(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if id != 0 {
		t.Errorf("got id %d, want 0", id)
	} else if err.Error() != "[Filter, Where] not supported for Create" {
	...
	if id, err := ctl(ctx).Filter(Cond{}).Where("").OrderBy("").Create(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if id != 0 {
		t.Errorf("got id %d, want 0", id)
	} else if err.Error() != "[Filter, Where, OrderBy] not supported for Create" {
	...
	if id, err := ctl(ctx).Filter(Cond{}).Where("").OrderBy("").GroupBy("").Create(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if id != 0 {
		t.Errorf("got id %d, want 0", id)
	} else if err.Error() != "[Filter, Where, OrderBy, GroupBy] not supported for Create" {
	...
	if id, err := ctl(ctx).Filter(Cond{}).Where("").OrderBy("").GroupBy("").Select("").Create(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if id != 0 {
		t.Errorf("got id %d, want 0", id)
	} else if err.Error() != "[Filter, Where, Select, OrderBy, GroupBy] not supported for Create" {
	...
	if num, err := ctl(ctx).GroupBy("").Update(map[string]any{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if num != 0 {
		t.Errorf("got num %d, want 0", num)
	} else if err.Error() != "[GroupBy] not supported for Update" {
		t.Errorf("got %q, want %q", err.Error(), "[GroupBy] not supported for Update")
	}

	if num, err := ctl(ctx).GroupBy("").Select("").Update(map[string]any{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if num != 0 {
		t.Errorf("got num %d, want 0", num)
	} else if err.Error() != "[Select, GroupBy] not supported for Update" {
		t.Errorf("got %q, want %q", err.Error(), "[Select, GroupBy] not supported for Update")
	}

	// Remove unsupported operations
	if num, err := ctl(ctx).GroupBy("").Remove(); err == nil {
		t.Fatal("expected error, got nil")
	} else if num != 0 {
		t.Errorf("got num %d, want 0", num)
	} else if err.Error() != "[GroupBy] not supported for Remove" {
		t.Errorf("got %q, want %q", err.Error(), "[GroupBy] not supported for Remove")
	}

	if num, err := ctl(ctx).GroupBy("").Select("").Remove(); err == nil {
		t.Fatal("expected error, got nil")
	} else if num != 0 {
		t.Errorf("got num %d, want 0", num)
	} else if err.Error() != "[Select, GroupBy] not supported for Remove" {
		t.Errorf("got %q, want %q", err.Error(), "[Select, GroupBy] not supported for Remove")
	}

	// Delete unsupported operations
	if num, err := ctl(ctx).GroupBy("").Delete(); err == nil {
		t.Fatal("expected error, got nil")
	} else if num != 0 {
		t.Errorf("got num %d, want 0", num)
	} else if err.Error() != "[GroupBy] not supported for Delete" {
		t.Errorf("got %q, want %q", err.Error(), "[GroupBy] not supported for Delete")
	}

	if num, err := ctl(ctx).GroupBy("").Select("").Delete(); err == nil {
		t.Fatal("expected error, got nil")
	} else if num != 0 {
		t.Errorf("got num %d, want 0", num)
	} else if err.Error() != "[Select, GroupBy] not supported for Delete" {
		t.Errorf("got %q, want %q", err.Error(), "[Select, GroupBy] not supported for Delete")
	}

	// Exist unsupported operations
	if _, err := ctl(ctx).GroupBy("").Exist(); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != fmt.Errorf(UnsupportedControllerError, ctlGroupBy.Name, "Exist").Error() {
		t.Errorf("got %q, want %q", err.Error(), fmt.Errorf(UnsupportedControllerError, ctlGroupBy.Name, "Exist").Error())
	}

	if _, err := ctl(ctx).Select("").Exist(); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != fmt.Errorf(UnsupportedControllerError, ctlSelect.Name, "Exist").Error() {
		t.Errorf("got %q, want %q", err.Error(), fmt.Errorf(UnsupportedControllerError, ctlSelect.Name, "Exist").Error())
	}

	// GetOrCreate unsupported operations
	if _, err := ctl(ctx).GroupBy("").Having("").GetOrCreate(map[string]any{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != fmt.Errorf(UnsupportedControllerError, strings.Join([]string{ctlGroupBy.Name, ctlHaving.Name}, ", "), "GetOrCreate").Error() {
		t.Errorf("got %q, want %q", err.Error(), fmt.Errorf(UnsupportedControllerError, strings.Join([]string{ctlGroupBy.Name, ctlHaving.Name}, ", "), "GetOrCreate").Error())
	}

	if _, err := ctl(ctx).Select("").GetOrCreate(map[string]any{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != fmt.Errorf(UnsupportedControllerError, ctlSelect.Name, "GetOrCreate").Error() {
		t.Errorf("got %q, want %q", err.Error(), fmt.Errorf(UnsupportedControllerError, ctlSelect.Name, "GetOrCreate").Error())
	}

	// CreateOrUpdate unsupported operations
	if _, _, err := ctl(ctx).GroupBy("").Having("").CreateOrUpdate(map[string]any{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != fmt.Errorf(UnsupportedControllerError, strings.Join([]string{ctlGroupBy.Name, ctlHaving.Name}, ", "), "CreateOrUpdate").Error() {
		t.Errorf("got %q, want %q", err.Error(), fmt.Errorf(UnsupportedControllerError, strings.Join([]string{ctlGroupBy.Name, ctlHaving.Name}, ", "), "CreateOrUpdate").Error())
	}

	if _, _, err := ctl(ctx).Select("").CreateOrUpdate(map[string]any{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != fmt.Errorf(UnsupportedControllerError, ctlSelect.Name, "CreateOrUpdate").Error() {
		t.Errorf("got %q, want %q", err.Error(), fmt.Errorf(UnsupportedControllerError, ctlSelect.Name, "CreateOrUpdate").Error())
	}

	// CreateIfNotExists unsupported operations
	if _, _, err := ctl(ctx).GroupBy("").Having("").CreateIfNotExist(map[string]any{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != fmt.Errorf(UnsupportedControllerError, strings.Join([]string{ctlGroupBy.Name, ctlHaving.Name}, ", "), "CreateIfNotExist").Error() {
		t.Errorf("got %q, want %q", err.Error(), fmt.Errorf(UnsupportedControllerError, strings.Join([]string{ctlGroupBy.Name, ctlHaving.Name}, ", "), "CreateIfNotExist").Error())
	}

	if _, _, err := ctl(ctx).Select("").CreateIfNotExist(map[string]any{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != fmt.Errorf(UnsupportedControllerError, ctlSelect.Name, "CreateIfNotExist").Error() {
		t.Errorf("got %q, want %q", err.Error(), fmt.Errorf(UnsupportedControllerError, ctlSelect.Name, "CreateIfNotExist").Error())
	}

	// send not exist columns to Select
	if err := ctl(ctx).Select([]string{"age"}).FindOneModel(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "select columns validate error: [age] not exist" {
		t.Errorf("got %q, want %q", err.Error(), "select columns validate error: [age] not exist")
	}

	if err := ctl(ctx).Select([]string{"age", "happy"}).FindOneModel(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "select columns validate error: [age; happy] not exist" {
		t.Errorf("got %q, want %q", err.Error(), "select columns validate error: [age; happy] not exist")
	}

	if err := ctl(ctx).Select([]string{"age", "happy", "damnit"}).FindOneModel(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "select columns validate error: [age; happy; damnit] not exist" {
		t.Errorf("got %q, want %q", err.Error(), "select columns validate error: [age; happy; damnit] not exist")
	}

	// send not exist columns to OrderBy
	if err := ctl(ctx).OrderBy([]string{"age"}).FindOneModel(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "orderBy columns validate error: [age] not exist" {
		t.Errorf("got %q, want %q", err.Error(), "orderBy columns validate error: [age] not exist")
	}

	if err := ctl(ctx).OrderBy([]string{"age", "happy"}).FindOneModel(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "orderBy columns validate error: [age; happy] not exist" {
		t.Errorf("got %q, want %q", err.Error(), "orderBy columns validate error: [age; happy] not exist")
	}

	if err := ctl(ctx).OrderBy([]string{"age", "happy", "damnit"}).FindOneModel(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "orderBy columns validate error: [age; happy; damnit] not exist" {
		t.Errorf("got %q, want %q", err.Error(), "orderBy columns validate error: [age; happy; damnit] not exist")
	}

	// send not exist columns to GroupBy
	if err := ctl(ctx).GroupBy([]string{"age"}).FindOneModel(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "groupBy columns validate error: [age] not exist" {
		t.Errorf("got %q, want %q", err.Error(), "groupBy columns validate error: [age] not exist")
	}

	if err := ctl(ctx).GroupBy([]string{"age", "happy"}).FindOneModel(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "groupBy columns validate error: [age; happy] not exist" {
		t.Errorf("got %q, want %q", err.Error(), "groupBy columns validate error: [age; happy] not exist")
	}

	if err := ctl(ctx).GroupBy([]string{"age", "happy", "damnit"}).FindOneModel(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "groupBy columns validate error: [age; happy; damnit] not exist" {
		t.Errorf("got %q, want %q", err.Error(), "groupBy columns validate error: [age; happy; damnit] not exist")
	}
	...
	if err := ctl(ctx).OrderBy([]string{"age"}).GroupBy([]string{"test"}).FindOneModel(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "groupBy columns validate error: [test] not exist" {
		t.Errorf("got %q, want %q", err.Error(), "groupBy columns validate error: [test] not exist")
	}

	if err := ctl(ctx).OrderBy([]string{"age", "happy"}).GroupBy([]string{"test", "test2"}).FindOneModel(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "groupBy columns validate error: [test; test2] not exist" {
		t.Errorf("got %q, want %q", err.Error(), "groupBy columns validate error: [test; test2] not exist")
	}

	if err := ctl(ctx).OrderBy([]string{"age", "happy", "damnit"}).GroupBy([]string{"test", "test2", "test3"}).FindOneModel(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "groupBy columns validate error: [test; test2; test3] not exist" {
		t.Errorf("got %q, want %q", err.Error(), "groupBy columns validate error: [test; test2; test3] not exist")
	}
	...
	if _, err := cli.Create(map[string]any{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "[Filter] not supported for Create" {
		t.Errorf("got %q, want %q", err.Error(), "[Filter] not supported for Create")
	}
	...
	if _, err := cli.Create(map[string]any{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "create data is empty" {
	...
	if err := ctl(ctx).Select([]int{1}).FindOneModel(&test.Source{}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != "select type should be string or string slice" {
		t.Errorf("got %q, want %q", err.Error(), "select type should be string or string slice")
	}

	// orderBy type error
	if _, err := ctl(ctx).OrderBy([1]string{"id"}).FindAll(); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != OrderByColumnsTypeError {
		t.Errorf("got %q, want %q", err.Error(), OrderByColumnsTypeError)
	}
	...
	if err := ctl(ctx).Select([]string{"name"}).GroupBy([1]string{"name"}).FindAllModel(&[]struct {
		...
		t.Fatal("expected error, got nil")
	} else if err.Error() != GroupByColumnsTypeError {
		t.Errorf("got %q, want %q", err.Error(), GroupByColumnsTypeError)
	}
	...
	if _, err := ctl(ctx).Filter(nil).Count(); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != fmt.Errorf(queryset.UnsupportedFilterTypeError, "nil").Error() {
		t.Errorf("got %q, want %q", err.Error(), fmt.Errorf(queryset.UnsupportedFilterTypeError, "nil").Error())
	}
	...
	if _, err := ctl(ctx).Filter(Cond{"id": 11}).Update(map[string]any{"name": "test", "age": 18}); err == nil {
		t.Fatal("expected error, got nil")
	} else if err.Error() != fmt.Errorf(UpdateColumnNotExistError, "age").Error() {
		t.Errorf("got %q, want %q", err.Error(), fmt.Errorf(UpdateColumnNotExistError, "age").Error())
	}
}

// ============================================================================
// 以下是优化后的测试用例，保留原测试用例在上面方便对比
// ============================================================================
