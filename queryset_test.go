...
func TestFilter(t *testing.T) {
	...
	tests := []struct {
		...
		{"list_contains_cond", args{isFilter, []any{Cond{"test__contains": []string{"e", "s"}}}}, want{" WHERE (`test` LIKE BINARY ? OR `test` LIKE BINARY ?)", []any{"%e%", "%s%"}}},
		{"not_contains_cond", args{isFilter, []any{Cond{"test__not_contains": "e"}}}, want{" WHERE (`test` NOT LIKE BINARY ?)", []any{"%e%"}}},
		{"icontains_cond", args{isFilter, []any{Cond{"test__icontains": "E"}}}, want{" WHERE (`test` LIKE ?)", []any{"%E%"}}},
		{"not_icontains_cond", args{isFilter, []any{Cond{"test__not_icontains": "E"}}}, want{" WHERE (`test` NOT LIKE ?)", []any{"%E%"}}},
		{"startswith_cond", args{isFilter, []any{Cond{"test__startswith": "te"}}}, want{" WHERE (`test` LIKE BINARY ?)", []any{"te%"}}},
		{"not_startswith_cond", args{isFilter, []any{Cond{"test__not_startswith": "te"}}}, want{" WHERE (`test` NOT LIKE BINARY ?)", []any{"te%"}}},
		{"istartswith_cond", args{isFilter, []any{Cond{"test__istartswith": "tE"}}}, want{" WHERE (`test` LIKE ?)", []any{"tE%"}}},
		{"not_istartswith_cond", args{isFilter, []any{Cond{"test__not_istartswith": "tE"}}}, want{" WHERE (`test` NOT LIKE ?)", []any{"tE%"}}},
